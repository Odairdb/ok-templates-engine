...
export default function CorpFooter({ niche }: { niche: string }) {
  const getLogoText = () => {
    switch(niche) {
  ...
  return (
    <footer style={{ backgroundColor: '#0B132B', color: '#FFFFFF', padding: '80px 24px 40px 24px' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '48px', marginBottom: '80px' }}>
          
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#FFFFFF', marginBottom: '24px' }}>
              <Hexagon size={32} color="#8D99AE" strokeWidth={2.5} />
            ...
            <div style={{ display: 'flex', gap: '16px' }}>
              <a href="#" style={{ color: '#FFFFFF', opacity: 0.7, transition: 'opacity 0.2s' }} onMouseOver={(e) => e.currentTarget.style.opacity = '1'} onMouseOut={(e) => e.currentTarget.style.opacity = '0.7'}>
                <Globe size={24} />
              </a>
            </div>
          </div>
          ...
          <div>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <li style={{ display: 'flex', gap: '12px', color: 'rgba(255,255,255,0.7)' }}>
                <MapPin size={20} color="#8D99AE" style={{ flexShrink: 0 }} />
                <span>Av. Paulista, 1000 - Bela Vista<br/>São Paulo - SP</span>
              </li>
              <li style={{ display: 'flex', gap: '12px', color: 'rgba(255,255,255,0.7)' }}>
                <Phone size={20} color="#8D99AE" style={{ flexShrink: 0 }} />
                <span>(11) 3333-4444</span>
              </li>
              <li style={{ display: 'flex', gap: '12px', color: 'rgba(255,255,255,0.7)' }}>
                <Mail size={20} color="#8D99AE" style={{ flexShrink: 0 }} />
                <span>contato@Corpinc.com.br</span>
              </li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {['Projetos', 'Especialidades', 'Metodologia', 'Fale Conosco'].map(link => (
                <li key={link}>
                  <a href={`#${link.toLowerCase().replace(' ', '-')}`} style={{ color: 'rgba(255,255,255,0.7)', textDecoration: 'none', transition: 'color 0.2s' }} onMouseOver={(e) => e.currentTarget.style.color = '#fff'} onMouseOut={(e) => e.currentTarget.style.color = 'rgba(255,255,255,0.7)'}>
                    {link}
                  </a>
                </li>
              ))}
